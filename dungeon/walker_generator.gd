...
func generate_map() -> void:
	if tilemap_layer == null:
		push_error("WalkerGenerator: tilemap_layer non assegnato nell'Inspector!")
		return
	...
	_floor_cells = _walk_and_collect(map_dimensions, boundary_padding)

	for cell in _floor_cells:
		tilemap_layer.set_cell(cell, TILE_DATA.floor.source_id, TILE_DATA.floor.atlas_coords)

	print("WalkerGenerator: generati %d floor cells" % _floor_cells.size())
...
func _walk_and_collect(dimensions: Vector2i, padding: int) -> Array[Vector2i]:
	...
	var cur_pos := Vector2i(dimensions.x / 2, dimensions.y / 2)
	var bounds := Rect2i(0, 0, dimensions.x, dimensions.y)
...
func place_spawn_and_exit() -> void:
	if _floor_cells.is_empty():
		push_error("WalkerGenerator: _floor_cells vuoto, generate_map() non è stato chiamato o è fallito.")
		return

	var spawn_cell: Vector2i = _floor_cells[0]
